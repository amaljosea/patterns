/**
 * To format the query to support gatsby-wiki-theme implementation
 */
const formatForWiki = (pages)=> {
  return pages.map((page)=>({
    node:{
      frontmatter:{
        path: page.node.fields.slug.slice(0,-1),
...
const parseLinksToTree = pages => {
  ...
  const navTree = formatForWiki(pages).reduce(
...
export { parseLinksToTree, formatForWiki };
